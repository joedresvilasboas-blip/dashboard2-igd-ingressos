...
const Vendedor = {
  _dados: null,       // resposta completa do backend (sempre 8 semanas)
  _periodoSel: null,  // null = tudo | { tipo:'mes', nome } | { tipo:'sem', semana }
  ...
  // Chamado pelos botões de filtro — sem nova chamada à API
  selecionarPeriodo(tipo, valor) {
    if (tipo === 'tudo') {
      this._periodoSel = null;
    } else if (tipo === 'mes') {
      this._periodoSel = { tipo: 'mes', nome: valor };
    } else {
  ...
  _psFiltrado() {
    ...
    if (sel.tipo === 'sem') {
      return ps.filter(s => s.semana === sel.semana);
    }
  ...
  // Recalcula totais a partir das semanas filtradas
  _agregarTotais(ps) {
    const t = {
      hc:0, va:0, rc:0, upgrade:0, cancelados:0, pontos:0, faturamento:0,
      atend:0, entrv:0, horasSeg:0, diasAtivos:0,
    };
    ps.forEach(s => {
      t.hc          += s.hc;
      t.va          += s.va;
      t.rc          += s.rc;
      t.upgrade     += s.upgrade;
      t.cancelados  += s.cancelados;
      t.pontos      += s.pontos;
      t.faturamento += (s.faturamento || 0);
      t.atend       += s.atend;
      t.entrv       += s.entrv;
      t.horasSeg    += s.horasSeg;
      t.diasAtivos  += s.diasAtivos;
    });
  ...
  _labelPeriodo() {
    const sel = this._periodoSel;
    if (!sel) return 'Últimas ' + (this._dados.porSemana || []).length + ' semanas';
    if (sel.tipo === 'mes') return sel.nome;
    if (sel.tipo === 'sem') return 'Semana ' + sel.semana;
    return '';
  },

  // Monta listas de meses e semanas disponíveis nos dados carregados
  _periodosDisponiveis() {
    const ps  = this._dados.porSemana || [];
    const cfg = (typeof App !== 'undefined' && App._config) || {};
    const cal = cfg.semanas || [];

    // Semanas que existem nos dados
    const numsSem = new Set(ps.map(s => s.semana));

    // Meses: na ordem do calendário, só se tiver ao menos 1 semana nos dados
    const mesesVistos = new Set();
    const meses = [];
    cal.forEach(s => {
      if (s.mes && numsSem.has(s.num) && !mesesVistos.has(s.mes)) {
        mesesVistos.add(s.mes);
        meses.push(s.mes);
      }
    });

    const semanas = ps.map(s => s.semana);
  ...
  _renderTela() {
    ...
    const sel = this._periodoSel;
    const { meses, semanas } = this._periodosDisponiveis();

    const btn = (label, tipo, valor) => {
      const ativo = tipo === 'tudo' ? !sel
        : tipo === 'mes' ? (sel && sel.tipo === 'mes' && sel.nome === valor)
        : (sel && sel.tipo === 'sem' && sel.semana === parseInt(valor));
      const cor = ativo ? 'var(--accent)' : 'var(--text-3)';
      const bg  = ativo ? 'var(--accent-dim)' : 'transparent';
      const bd  = ativo ? 'var(--accent)' : 'var(--border)';
      const fn  = tipo === 'tudo' ? `Vendedor.selecionarPeriodo('tudo')`
        : tipo === 'mes' ? `Vendedor.selecionarPeriodo('mes','${valor}')`
        : `Vendedor.selecionarPeriodo('sem',${valor})`;
      return `<button onclick="${fn}"
        style="padding:3px 11px;border-radius:20px;border:1px solid ${bd};background:${bg};color:${cor};font-size:11px;cursor:pointer;white-space:nowrap;flex-shrink:0">${label}</button>`;
    };
      ...
      <div style="padding:var(--s4) var(--s5)">
        ...
        <div class="card" style="margin-bottom:var(--s4)">
          ...
          <div style="margin-top:var(--s3);padding-top:var(--s3);border-top:1px solid var(--border)">
            <div style="font-size:10px;color:var(--text-3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Período</div>
            <!-- Linha 1: Tudo + Meses -->
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px">
              ${btn('Tudo', 'tudo', null)}
              ${meses.map(m => btn(m, 'mes', m)).join('')}
            </div>
            <!-- Linha 2: Semanas individuais -->
            <div style="display:flex;gap:6px;flex-wrap:wrap">
              ${semanas.map(n => btn('S'+n, 'sem', n)).join('')}
            </div>
          </div>
  ...
  _blocoROI(v, t, ps) {
    ...
    // Custo fixo: mês fechado = R$ cheio; períodos parciais = proporcional por semana
    const custoFixoCalc = isMesFechado
      ? fixo
      : Math.round(fixo / 4 * Math.max(ps.length, 1));
    ...
    const breakeven  = faturamento > 0 && t.hc > 0
      ? Math.ceil(custoTotal / (faturamento / t.hc)) : null;
    ...
    // Projeção mensal só quando não é mês fechado
    let projecao = '';
      ...
      projecao = `
        <div style="margin-top:var(--s3);padding:var(--s3);background:var(--bg-3);border-radius:var(--r2)">
          ...
          <div style="display:flex;gap:var(--s4)">
            <div>
              <div style="font-size:15px;font-weight:700;color:var(--text)">${Utils.moeda(faturMensal)}</div>
              <div style="font-size:9px;color:var(--text-3)">Faturamento proj.</div>
            </div>
            <div>
              <div style="font-size:15px;font-weight:700;color:var(--text)">${Utils.moeda(custoMensal)}</div>
              <div style="font-size:9px;color:var(--text-3)">Custo proj.</div>
            </div>
            <div>
              <div style="font-size:15px;font-weight:700;color:${roiMensal>=0?'#4caf50':'#e85d5d'}">${roiMensal}%</div>
              <div style="font-size:9px;color:var(--text-3)">ROI mensal proj.</div>
            </div>
          </div>
        </div>`;
    }
